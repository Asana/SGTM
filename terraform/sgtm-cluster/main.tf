...
resource "aws_api_gateway_method_settings" "sgtm_api_settings" {
  rest_api_id = var.sgtm_rest_api_id
  stage_name  = aws_api_gateway_deployment.sgtm_deployment.stage_name
  method_path = "*/*"
  settings {
    logging_level = "INFO"
...
resource "aws_sqs_queue" "sgtm-webhooks-fifo" {
  name                        = "sgtm-webhooks.fifo" #todo add suffix
  fifo_queue                  = true
...
resource "aws_iam_role" "iam_for_api_gateway" {
  name = "iam_for_api_gateway${local.suffix}"
