...
class TestGithubClientGetReviewForDatabaseId(BaseClass):
    ...
    def test_when_no_reviews_found__should_raise_error(self, mock_query):
        ...
        with self.assertRaises(ValueError):
            actual = client.get_review_for_database_id(self.PULL_REQUEST_ID, self.REVIEW_DB_ID)

        mock_query.assert_called_once_with("IterateReviews", {"pullRequestId": self.PULL_REQUEST_ID})

    def test_when_no_reviews_match__should_raise_error(self, mock_query):
        mock_query.side_effect = [
            {"node": {"reviews": {"edges": [{'cursor': 'some-cursor', 'node': {'databaseId': 'doesnt match'}}]}}},
            {"node": {"reviews": {"edges": []}}}
        ]
        ...
        mock_query.assert_has_calls([
            ...
            call("IterateReviews", {"pullRequestId": self.PULL_REQUEST_ID, "cursor": "some-cursor"})
        ])
    ...
    def test_when_review_in_first_batch_matches__should_return_it(self, mock_query):
        ...
        mock_query.side_effect = [
            {"node": {"reviews": {"edges": [{'cursor': 'some-cursor', 'node': other_node},
    ...
    def test_when_review_in_second_batch_matches__should_return_it(self, mock_query):
        ...
        other_node = {'id': 'other-review', 'databaseId': 'doesnt match'}
        mock_query.side_effect = [
            {"node": {"reviews": {"edges": [{'cursor': 'cursor-1', 'node': other_node}]}}},
            {"node": {"reviews": {"edges": [{'cursor': 'cursor-2', 'node': matching_node}]}}},
            {"node": {"reviews": {"edges": []}}}
        ]
        actual = client.get_review_for_database_id(self.PULL_REQUEST_ID, self.REVIEW_DB_ID)
        ...
        mock_query.assert_has_calls([
            call("IterateReviews", {"pullRequestId": self.PULL_REQUEST_ID}),
            call("IterateReviews", {"pullRequestId": self.PULL_REQUEST_ID, "cursor": "cursor-1"})
        ])
        self.assertEqual(mock_query.call_count, 2)
